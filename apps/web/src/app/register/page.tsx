...
function RegisterForm() {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.success('Welcome to Pizza Height!', {
  ...
  return (
    <AuthShell
      title="Join the height"
      subtitle="A few details and you’re ready to order luxury pizza."
      switchPrompt="Already a member?"
      switchLabel="Sign in"
      switchHref={`/login${redirectTo !== '/menu' ? `?next=${encodeURIComponent(redirectTo)}` : ''}`}
    >
      <form onSubmit={onSubmit} className="space-y-5">
        ...
        <Field label="Phone number" hint="We'll use this to confirm your orders.">
          <TextInput
            type="tel"
            inputMode="tel"
            autoComplete="tel"
